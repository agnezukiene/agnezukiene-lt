...
const technicalGates = [
  ...
  checked(has("scripts/check-analytics-privacy.js", "withdrawing consent should preserve keyboard focus and reload without analytics") && has("public/assets/js/site.js", "window.location.reload()"), "Lankomumo sutikimo atšaukimas sustabdo matavimą, pašalina jo slapukus ir iš naujo atveria puslapį be lankomumo įrankio"),
  ...
  checked(has("scripts/check-site-integrity.js", "has no matching id") && has("scripts/check-live-site.js", "missing direct link to service section"), "Paslaugų kortelės nuveda į tikslią pasirinktą paslaugos vietą"),
  checked(has("public/assets/css/styles.css", "scroll-margin-top: 6rem") && has("scripts/check-live-site.js", "linked service sections should clear the sticky header"), "Pasirinktos paslaugos antraštės neuždengia prilipęs viršutinis meniu"),
  checked(has("scripts/check-site-integrity.js", "new-window link should explain its behavior") && has("scripts/check-live-site.js", "new-window link should announce its behavior"), "Pagalbinė skaitymo priemonė perspėja apie naujame lange atidaromas nuorodas"),
  checked(has("scripts/check-site-integrity.js", "footer should include the safe D2.lt creator credit") && has("scripts/check-live-site.js", "/neegzistuojantis-puslapis: missing safe D2.lt creator credit"), "Sprendimo autoriaus D2.lt nuoroda rodoma visuose puslapiuose, įskaitant klaidos puslapį"),
  checked(has("public/index.html", '<script data-early-js>document.documentElement.classList.add("has-js");</script>') && has("public/assets/css/styles.css", ".has-js .nav-links"), "Telefono navigacija lieka pasiekiama, jei papildomas svetainės kodas neįsikrauna"),
  checked(has("scripts/check-site-integrity.js", "font sizes should use stable type scales") && has("public/assets/css/styles.css", "overflow-wrap: anywhere") && !/font-size:\s*[^;]*(?:vw|vh)/.test(read("public/assets/css/styles.css")), "Antraščių dydžiai stabilūs, o ilgi žodžiai neišplečia siauro ekrano"),
  ...
  checked(has("scripts/check-site-integrity.js", "max-image-preview:large") && has("scripts/check-live-site.js", "large image previews"), "Paieškos sistemoms leidžiama rodyti didesnę Agnės nuotraukos peržiūrą"),
  checked(has("scripts/check-site-integrity.js", "FAQ structured data should exactly match") && has("scripts/check-live-site.js", "structured questions should match visible content"), "Dažnų klausimų paieškos aprašas tiksliai sutampa su visais lankytojui matomais klausimais ir atsakymais"),
  checked(has("scripts/check-site-integrity.js", "main content should provide a direct, measurable contact action") && has("scripts/check-live-site.js", "main content should provide a direct contact action"), "Kiekvienas pagrindinis informacinis puslapis po turinio pateikia tiesioginį kelią į kontaktų formą"),
  ...
  checked(has("public/assets/js/site.js", "showFieldError") && has("public/kontaktai.html", "field-error") && has("public/kontaktai.html", "contact-method-error"), "Formos klaidos rodomos prie konkretaus lauko ir perskaitomos pagalbinėmis priemonėmis"),
  ...
  checked(has("public/assets/js/site.js", "cookieChoiceReturnFocus") && has("public/assets/js/site.js", "preventScroll") && has("public/slapuku-politika.html", "agne_cookie_reset_pending"), "Keičiant slapukų pasirinkimą klaviatūros žymeklis pereina prie pasirinkimų ir grįžta į ankstesnę vietą"),
